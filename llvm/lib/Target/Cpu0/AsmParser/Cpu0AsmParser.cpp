//===-- Cpu0AsmParser.cpp - Parse Cpu0 assembly to MCInst instructions - C++ -//
//
...
public:
  Cpu0AssemblerOptions() : reorder(true), macro(true) {}
...
#include "Cpu0GenAsmMatcher.inc"
  ...
  ParseStatus parseMemOperand(OperandVector &);
...
public:
  ...
  static std::unique_ptr<Cpu0Operand>
...
void Cpu0AsmParser::expandLoadAddressReg(
...
void Cpu0AsmParser::expandLoadAddressImm(
    MCInst &Inst, SMLoc IDLoc, SmallVectorImpl<MCInst> &Instructions) {
  MCInst tmpInst;
  const MCOperand &ImmOp = Inst.getOperand(1);
  assert(ImmOp.isImm() && "expected immediate operand kind");
...
bool Cpu0AsmParser::tryParseRegisterOperand(OperandVector &Operands,
  ...
  Operands.push_back(
      Cpu0Operand::CreateReg(RegNo, S, Parser.getTok().getLoc()));
...
bool Cpu0AsmParser::ParseOperand(OperandVector &Operands, StringRef Mnemonic) {
  ...
  case AsmToken::Dollar: {
    ...
    const MCExpr *Res = MCSymbolRefExpr::create(Sym, 0, getContext());
...
ParseStatus Cpu0AsmParser::tryParseRegister(MCRegister &RegNo, SMLoc &StartLoc,
                                            SMLoc &EndLoc) {
  StartLoc = Parser.getTok().getLoc();
  RegNo = tryParseRegister("");
  EndLoc = Parser.getTok().getLoc();
  return (RegNo == (unsigned)-1) ? ParseStatus::NoMatch : ParseStatus::Success;
}
...
// eg, 12($sp) or 12(la)
ParseStatus Cpu0AsmParser::parseMemOperand(OperandVector &Operands) {
