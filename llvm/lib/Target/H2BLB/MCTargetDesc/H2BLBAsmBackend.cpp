...
void H2BLBAsmBackend::applyFixup(const MCFragment &, const MCFixup &Fixup,
...
MCFixupKindInfo H2BLBAsmBackend::getFixupKindInfo(MCFixupKind Kind) const {
  const static MCFixupKindInfo Infos[H2BLB::NumTargetFixupKinds] = {
      // name                    , offset, bits, flags
      {"fixup_h2blb_pcrel_call11",      0,   11, 0},
  };

  if (Kind < FirstTargetFixupKind)
    return MCAsmBackend::getFixupKindInfo(Kind);

  assert(unsigned(Kind - FirstTargetFixupKind) < H2BLB::NumTargetFixupKinds &&
         "Invalid kind!");
