...
Register OR1KTargetLowering::getExceptionPointerRegister(
    const Constant *PersonalityFn) const {
  return OR1K::R3;
}

Register OR1KTargetLowering::getExceptionSelectorRegister(
    const Constant *PersonalityFn) const {
  return OR1K::R4;
}
  ...
  case 'I': // Signed 16 bit constant
    ...
    if (ConstantSDNode *C = dyn_cast<ConstantSDNode>(Op)) {
      if (isInt<16>(C->getSExtValue())) {
        Result =
            DAG.getTargetConstant(C->getSExtValue(), dl, Op.getValueType());
        break;
      }
    }
    return;
  ...
  case 'L': // immediate in the range 0 to 31
    if (ConstantSDNode *C = dyn_cast<ConstantSDNode>(Op)) {
      if (C->getZExtValue() <= 31) {
        Result =
            DAG.getTargetConstant(C->getZExtValue(), dl, Op.getValueType());
        break;
      }
    }
    return;
...
static bool CC_OR1K32_VarArg(unsigned ValNo, MVT ValVT, MVT LocVT,
...
SDValue OR1KTargetLowering::LowerCCCArguments(
    SDValue Chain, CallingConv::ID CallConv, bool isVarArg,
...
SDValue OR1KTargetLowering::LowerCCCCallTo(
  ...
  if (IsPIC) {
    ...
    Chain =
        DAG.getCopyToReg(Chain, dl, GPReg, getGlobalReg(DAG, PtrVT), InFlag);
  ...
  if (G) {
    ...
    Callee = DAG.getTargetExternalSymbol(E->getSymbol(), PtrVT, OpFlag);
  }
...
SDValue OR1KTargetLowering::LowerBR_CC(SDValue Op, SelectionDAG &DAG) const {
  ...
  SDValue Flag = DAG.getNode(OR1KISD::SET_FLAG, dl, MVT::Glue, LHS, RHS,
                             DAG.getConstant(CC, dl, MVT::i32));

  return DAG.getNode(OR1KISD::BR_CC, dl, Op.getValueType(), Chain, Dest, Neg,
                     Flag);
}
...
SDValue OR1KTargetLowering::LowerSELECT_CC(SDValue Op,
  ...
  SDValue Flag =
      DAG.getNode(OR1KISD::SET_FLAG, dl, MVT::Glue, LHS, RHS, TargetCC);
...
SDValue OR1KTargetLowering::LowerDYNAMIC_STACKALLOC(SDValue Op,
                                                    SelectionDAG &DAG) const {
  SDValue Chain = Op.getOperand(0);
...
SDValue OR1KTargetLowering::LowerCTTZ(SDValue Op, SelectionDAG &DAG) const {
  ...
  return DAG.getNode(
      ISD::SELECT_CC, dl, VT, SrcReg, DAG.getConstant(0, dl, MVT::i32),
      DAG.getConstant(VT.getSizeInBits(), dl, MVT::i32), ZeroUndef, CC);
}
...
///
SDValue OR1KTargetLowering::LowerSUBE(SDValue Op, SelectionDAG &DAG) const {
  SDLoc dl(Op);
  EVT VT = Op.getValueType();
  SDValue LHS = Op.getOperand(0);
  SDValue RHS = Op.getOperand(1);
...
const char *OR1KTargetLowering::getTargetNodeName(unsigned Opcode) const {
  ...
  default:
    return NULL;
  case OR1KISD::ADJDYNALLOC:
    return "OR1KISD::ADJDYNALLOC";
  case OR1KISD::RET_FLAG:
    return "OR1KISD::RET_FLAG";
  case OR1KISD::CALL:
    return "OR1KISD::CALL";
  case OR1KISD::SELECT_CC:
    return "OR1KISD::SELECT_CC";
  case OR1KISD::SET_FLAG:
    return "OR1KISD::SET_FLAG";
  case OR1KISD::BR_CC:
    return "OR1KISD::BR_CC";
  case OR1KISD::Wrapper:
    return "OR1KISD::Wrapper";
  case OR1KISD::FF1:
    return "OR1KISD::FF1";
  case OR1KISD::FL1:
    return "OR1KISD::FL1";
...
SDValue OR1KTargetLowering::LowerGlobalAddress(SDValue Op,
  ...
  if (IsPIC && !GV->hasLocalLinkage() && !GV->hasHiddenVisibility()) {
    SDValue GA =
        DAG.getTargetGlobalAddress(GV, dl, PtrVT, Offset, OR1KII::MO_GOT);
...
SDValue OR1KTargetLowering::LowerBlockAddress(SDValue Op,
  ...
  SDValue Hi =
      DAG.getBlockAddress(BA, MVT::i32, 0, /*isTarget=*/true, OpFlagHi);
  SDValue Lo =
      DAG.getBlockAddress(BA, MVT::i32, 0, /*isTarget=*/true, OpFlagLo);
  Hi = DAG.getNode(OR1KISD::HI, dl, MVT::i32, Hi);
  Lo = DAG.getNode(OR1KISD::LO, dl, MVT::i32, Lo);
  SDValue Result = DAG.getNode(ISD::OR, dl, MVT::i32, Hi, Lo);
  if (IsPIC)
...
MachineBasicBlock *OR1KTargetLowering::emitSelect(MachineInstr &MI,
                                                  MachineBasicBlock *BB) const {
...
MachineBasicBlock *OR1KTargetLowering::emitAtomicBinary(MachineInstr &MI,
  ...
  if (Nand) {
    ...
    BuildMI(BB, DL, TII->get(OR1K::XOR), StoreVal)
        .addReg(AllOnes)
        .addReg(AndRes);
  } else if (BinOpcode) {
    //  <binop> storeval, oldval, incr
