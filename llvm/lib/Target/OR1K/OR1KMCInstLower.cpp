...
MCSymbol *
OR1KMCInstLower::GetGlobalAddressSymbol(const MachineOperand &MO) const {
  return Printer.getSymbol(MO.getGlobal());
}

MCSymbol *
OR1KMCInstLower::GetBlockAddressSymbol(const MachineOperand &MO) const {
  return Printer.GetBlockAddressSymbol(MO.getBlockAddress());
}

MCSymbol *
OR1KMCInstLower::GetExternalSymbolSymbol(const MachineOperand &MO) const {
  return Printer.GetExternalSymbolSymbol(MO.getSymbolName());
}

MCSymbol *OR1KMCInstLower::GetJumpTableSymbol(const MachineOperand &MO) const {
...
MCSymbol *
OR1KMCInstLower::GetConstantPoolIndexSymbol(const MachineOperand &MO) const {
  SmallString<256> Name;
  raw_svector_ostream(Name)
      << Printer.MAI->getPrivateGlobalPrefix() << "CPI"
      << Printer.getFunctionNumber() << '_' << MO.getIndex();
  // Create a symbol for the name.
  return Ctx.getOrCreateSymbol(Name);
}

MCOperand OR1KMCInstLower::LowerSymbolOperand(const MachineOperand &MO,
                                              MCSymbol *Sym) const {
  ...
  case OR1KII::MO_ABS_HI:
    Kind = MCSymbolRefExpr::VK_OR1K_ABS_HI;
    break;
  case OR1KII::MO_ABS_LO:
    Kind = MCSymbolRefExpr::VK_OR1K_ABS_LO;
    break;
  case OR1KII::MO_PLT:
    Kind = MCSymbolRefExpr::VK_OR1K_PLT;
    break;
  ...
  case OR1KII::MO_GOTOFFHI:
    Kind = MCSymbolRefExpr::VK_OR1K_GOTOFFHI;
    break;
  case OR1KII::MO_GOTOFFLO:
    Kind = MCSymbolRefExpr::VK_OR1K_GOTOFFLO;
    break;
  case OR1KII::MO_GOT:
    Kind = MCSymbolRefExpr::VK_OR1K_GOT;
    break;
  }
