...
class OR1KAsmParser : public MCTargetAsmParser {
  ...
  bool ParseRegister(MCRegister &RegNo, OperandVector &Operands);
...
public:
  ...
  static std::unique_ptr<OR1KOperand> CreateToken(StringRef Str, SMLoc S) {
    auto Op = std::make_unique<OR1KOperand>(k_Token);
  ...
  static std::unique_ptr<OR1KOperand> createReg(unsigned RegNo, SMLoc S,
                                                SMLoc E) {
    auto Op = std::make_unique<OR1KOperand>(k_Register);
    Op->Reg.Num = RegNo;
    Op->StartLoc = S;
    Op->EndLoc = E;
    return Op;
  ...
  static std::unique_ptr<OR1KOperand> createImm(const MCExpr *Val, SMLoc S,
                                                SMLoc E) {
    auto Op = std::make_unique<OR1KOperand>(k_Immediate);
    Op->Imm.Val = Val;
    Op->StartLoc = S;
    Op->EndLoc = E;
    return Op;
  ...
  CreateMem(unsigned BaseNum, const MCExpr *Off, SMLoc S, SMLoc E) {
    auto Op = std::make_unique<OR1KOperand>(k_Memory);
...
bool OR1KAsmParser::ParseRegister(MCRegister &RegNo, OperandVector &Operands) {
...
const MCExpr *
OR1KAsmParser::evaluateRelocExpr(const MCExpr *Expr,
                                 MCSymbolRefExpr::VariantKind VK) {
      ...
      if (getLexer().is(AsmToken::Identifier)) {
        ...
        if (BaseNum != 0) {
          ...
          if (getLexer().is(AsmToken::RParen)) {
            ...
            Operands.push_back(
                OR1KOperand::CreateMem(BaseNum, Off->getImm(), S, E));
            return false;
          }
        }
      }
...
bool OR1KAsmParser::ParseDirective(AsmToken DirectiveID) { return true; }
