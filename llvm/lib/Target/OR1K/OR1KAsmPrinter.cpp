...
void OR1KAsmPrinter::customEmitInstruction(const MachineInstr *MI) {
  OR1KMCInstLower MCInstLowering(OutContext, getObjFileLowering().getMangler(),
                                 *this);
  ...
  case OR1K::ORI: {
    ...
    MachineOperand MO = (MI->getOpcode() == OR1K::MOVHI) ? MI->getOperand(1)
                                                         : MI->getOperand(2);
    ...
    DotExpr = MCBinaryExpr::createAdd(
        MCSymbolRefExpr::create(OpSym, Kind, OutContext), DotExpr, OutContext);
  ...
  case OR1K::GETPC: {
    ...
    TmpInst.addOperand(
        MCOperand::createExpr(MCSymbolRefExpr::create(PICBase, OutContext)));
    OutStreamer->emitInstruction(TmpInst, STI);
